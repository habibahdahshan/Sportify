...
(function () {
    ...
    try {
      ...
      var normalized = data
        .map(function (item) {
          ...
          if (out.productId && window.sportifyGetProductById) {
            var p = window.sportifyGetProductById(out.productId);
            if (!p) return null;
            out.name = p.name;
            if (!out.currency) out.currency = p.currency || 'LE';
            if (window.sportifyResolveProductImage) out.image = window.sportifyResolveProductImage(p, out.image || p.image || '');
          } else if (window.sportifyResolveProductImage) {
            out.image = window.sportifyResolveProductImage(null, out.image || '');
          }
  ...
  function formatLinePrice(item) {
    ...
    if (window.sportifyFormatMoney) return window.sportifyFormatMoney(item.price * (item.qty || 1), sym);
    return sym + (item.price * (item.qty || 1)).toFixed(2);
  }

  function resolveImageForLine(item) {
    if (window.sportifyResolveProductImage) {
      return window.sportifyResolveProductImage(item.productId || null, item.image || '');
    }
    return item.image || '';
  }
  ...
  function renderCart() {
    ...
    items.forEach(function (item, index) {
      ...
      div.innerHTML =
        ...
        (item.size === '—' ? 'One size' : 'Size ' + escapeHtml(String(item.size))) +
        '</span>' +
        '</div>' +
    ...
    if (footer) {
      ...
      if (totalEl) {
        var parts = Object.keys(totals).map(function (sym) {
          if (window.sportifyFormatMoney) return window.sportifyFormatMoney(totals[sym], sym);
      ...
      if (checkoutBtn && !checkoutBtn.dataset.bound) {
        ...
        checkoutBtn.addEventListener('click', function () {
          ...
          if (window.sportifyRequireAuth && !window.sportifyRequireAuth('You need to sign in first to continue shopping.')) return;
          window.location.href = window.sportifyHtmlPage ? window.sportifyHtmlPage('checkout.html') : 'checkout.html';
        });
      }
    }
  }
  ...
  /**
   * addToCart(name, price, image?, size?, productId?, options?: { currency })
  ...
  function addToCart(name, price, image, size, productId, options) {
    ...
    var img = image || '';
    var chosenSize = size != null && size !== '' ? String(size) : '—';
    var pid = productId || null;
    if (pid && window.sportifyGetProductById) {
      var product = window.sportifyGetProductById(pid);
      if (product && product.inStock === false) return false;
      if (product && window.sportifyIsSizeInStock && chosenSize !== '—' && !window.sportifyIsSizeInStock(product, chosenSize)) return false;
      if (product) {
        name = product.name;
        currency = product.currency || currency;
        if (window.sportifyResolveProductImage) img = window.sportifyResolveProductImage(product, product.image || img);
      }
    }
    if (!pid && window.sportifyResolveProductImage) img = window.sportifyResolveProductImage(null, img);

    var items = getCart();
    var incoming = { name: name, price: Number(price), image: img, size: chosenSize, productId: pid, qty: addQty, currency: currency };
  ...
  function wireShopProductLinks() {
    document.querySelectorAll('.products-area .products-grid .card').forEach(function (card) {
      ...
      if (imgEl && window.sportifyGetShopIndexForImage) {
        var src = imgEl.getAttribute('src') || '';
        var file = src.split('/').pop().split('?')[0];
        idx = window.sportifyGetShopIndexForImage(file);
      }
      if (idx == null && window.sportifyGetShopIndexForName) {
        var h3El = card.querySelector('h3');
        var nameText = h3El ? h3El.textContent.trim() : '';
        idx = window.sportifyGetShopIndexForName(nameText);
      }
      if (idx == null) return;
      ...
      if (h3 && !h3.querySelector('a')) {
        h3.innerHTML = '<a href="' + (window.sportifyHtmlPage ? window.sportifyHtmlPage('product.html?shop=' + idx) : 'product.html?shop=' + idx) + '">' + h3.textContent.trim() + '</a>';
      }
      // Fix 9: also make the image clickable
      var imgWrap = card.querySelector('.image');
      if (imgWrap && !imgWrap.querySelector('a')) {
        var imgLink = document.createElement('a');
        imgLink.href = window.sportifyHtmlPage ? window.sportifyHtmlPage('product.html?shop=' + idx) : 'product.html?shop=' + idx;
  ...
  function sportifyAddFromShopCard(button) {
    var card = button && button.closest ? button.closest('.card') : null;
    if (!card) return;
    var pid = card.getAttribute('data-product-id') || null;
    if (pid && window.sportifyGetProductById) {
      var p = window.sportifyGetProductById(pid);
      if (p) {
        if (p.inStock === false) return;
        var pick = window.sportifyPickInStockSizeOption ? window.sportifyPickInStockSizeOption(p) : null;
        if (pick) {
          var added = addToCart(p.name, pick.price, p.image, String(pick.size), pid, { currency: p.currency || 'LE' });
    ...
    var sizeMatch = sub.match(/\b(S|M|L)\b/i);
    var size = sizeMatch ? sizeMatch[1].toUpperCase() : 'M';
    var added = addToCart(name, price, img, size, pid, { currency: 'LE' });
    if (added && window.toggleCart) window.toggleCart();
  }

  function getFavoriteIds() {
    try {
      var a = JSON.parse(localStorage.getItem('sportify_favorites') || '[]');
  ...
  function sportifyIsFavoriteId(id) {
    if (id == null || id === '') return false;
    return getFavoriteIds().indexOf(String(id)) !== -1;
  }

  function sportifyToggleFavoriteId(id) {
    if (id == null || id === '') return false;
    id = String(id);
  ...
  function renderFavoritesDrawer() {
    ...
    ids.forEach(function (id) {
      ...
      if (!p) return;
      var url = window.sportifyProductUrl ? window.sportifyProductUrl(p) : '#';
      var div = document.createElement('div');
      div.className = 'cart-item fav-drawer-item';
      var opts = window.sportifyGetSizeOptions ? window.sportifyGetSizeOptions(p) : [];
      var baseOpt = opts.find(function(x){ return x.size === p.baseSize; }) || opts[0];
      var priceStr = (baseOpt && window.sportifyFormatMoney) ? window.sportifyFormatMoney(baseOpt.price, p.currency || 'LE') : '';
      var canBuy = window.sportifyProductHasInStockSize ? window.sportifyProductHasInStockSize(p) : p.inStock !== false;
      div.innerHTML =
        '<img class="cart-item__thumb" src="' +
        String(window.sportifyResolveProductImage ? window.sportifyResolveProductImage(p, p.image) : p.image).replace(/"/g, '&quot;') +
        '" alt="" onerror="this.onerror=null;this.src=\'https://placehold.co/700x860/111111/FF4D1C?font=montserrat&text=Sportify\'">' +
    ...
    container.querySelectorAll('[data-fav-cart]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        var prod = window.sportifyGetProductById ? window.sportifyGetProductById(pid) : null;
        if (!prod) return;
        var pick2 = window.sportifyPickInStockSizeOption ? window.sportifyPickInStockSizeOption(prod) : null;
        if (!pick2) return;
        var added = addToCart(prod.name, pick2.price, prod.image, String(pick2.size), prod.id, { currency: prod.currency || 'LE' });
        if (!added) {
  ...
  function syncShopCardPricesLE() {
    if (!window.sportifyGetProductById || !window.sportifyFormatMoney) return;
    document.querySelectorAll('.card[data-product-id]').forEach(function (card) {
      var id = card.getAttribute('data-product-id');
      var p = window.sportifyGetProductById(id);
      var prEl = card.querySelector('.price');
      if (!p || !prEl || !p.sizes || p.sizes.length === 0) return;
      var base =
        p.sizes.find(function (s) {
          return s.size === p.baseSize;
        }) || p.sizes[p.sizes.length - 1];
      prEl.textContent = window.sportifyFormatMoney(base.price, p.currency || 'LE');
    });
  }

  function syncShopCardSizeLine() {
    if (!window.sportifyGetProductById) return;
    document.querySelectorAll('.card[data-product-id]').forEach(function (card) {
      var id = card.getAttribute('data-product-id');
      var p = window.sportifyGetProductById(id);
      ...
      sub.textContent = first ? first + ' · S / M / L' : 'Sportswear · S / M / L';

      var addBtn = card.querySelector('button');
      if (addBtn) {
        var out = window.sportifyProductHasInStockSize ? !window.sportifyProductHasInStockSize(p) : p.inStock === false;
    ...
    document.querySelectorAll('.card[data-product-id]').forEach(function (card) {
      ...
      if (qv && !qv.dataset.boundTools) {
        ...
        qv.addEventListener('click', function (e) {
          e.preventDefault();
          e.stopPropagation();
          if (window.sportifyOpenQuickView) window.sportifyOpenQuickView(id);
        });
      }
    });
  }
  ...
  window.sportifyGetCart = getCart;
  window.sportifySaveCart = saveCart;
  window.sportifyRefreshFavoritesDrawer = function () {
  ...
  function syncShopGridWithCatalog() {
    ...
    if (!kind || !window.sportifyGetProductsForShopPage) return;
    var grid = document.querySelector('.products-area .products-grid');
    if (!grid) return;

    var products = window.sportifyGetProductsForShopPage(kind);
    var allowedIds = {};
    ...
    products.forEach(function (p) {
      ...
      var url = window.sportifyProductUrl ? window.sportifyProductUrl(p) : 'product.html?id=' + encodeURIComponent(p.id);
      var line =
        p.line ||
      ...
      var base =
        opts.find(function (x) {
          return x.size === p.baseSize;
        }) || opts[0];
      var priceStr =
        base && window.sportifyFormatMoney ? window.sportifyFormatMoney(base.price, p.currency || 'LE') : '';
      var out = window.sportifyProductHasInStockSize && !window.sportifyProductHasInStockSize(p);
      ...
      var cardHtml =
        ...
        '" onerror="this.onerror=null;this.src=\'https://placehold.co/700x860/111111/FF4D1C?font=montserrat&text=Sportify\'"></a></div>' +
        '<h3><a href="' +
        url +
        '">' +
        escapeHtml(p.name) +
