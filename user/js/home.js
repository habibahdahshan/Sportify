...
function initHomeProductCart() {
  document.querySelectorAll('.product[data-product-id] .btn-product-add').forEach((btn) => {
    btn.addEventListener('click', function (e) {
      ...
      if (!pid || !window.sportifyGetProductById || !window.sportifyGetSizeOptions || !window.addToCart) return;
      const p = window.sportifyGetProductById(pid);
      if (!p) return;
      if (p.inStock === false) return;
      const o = window.sportifyPickInStockSizeOption?.(p);
      if (!o || o.inStock === false) return;
      const added = window.addToCart(p.name, o.price, p.image, String(o.size), p.id, { currency: p.currency || 'LE' });
...
function hydrateHomeStockButtons() {
  document.querySelectorAll('.product[data-product-id]').forEach((article) => {
    ...
    const p = window.sportifyGetProductById?.(id);
    const btn = article.querySelector('.btn-product-add');
    if (!p || !btn) return;
    const out = window.sportifyProductHasInStockSize ? !window.sportifyProductHasInStockSize(p) : p.inStock === false;
    btn.disabled = false;
...
function getFavorites() {
  try {
    const raw = localStorage.getItem('sportify_favorites');
...
function initProductHearts() {
  document.querySelectorAll('.product[data-product-id] .product__fav').forEach((btn) => {
    ...
    btn.addEventListener('click', (e) => {
      ...
      if (window.sportifyRefreshFavoritesDrawer) window.sportifyRefreshFavoritesDrawer();
    });
  });
}
...
function initQuickView() {
  document.querySelectorAll('.product[data-product-id] .product__qv').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      ...
      e.stopPropagation();
      const id = btn.closest('.product')?.getAttribute('data-product-id');
      if (id && window.sportifyOpenQuickView) window.sportifyOpenQuickView(id);
    });
  });
}
...
function renderFlashSaleFromShopCatalog() {
  ...
  if (!grid || !window.sportifyGetProductById || !window.sportifyProductUrl) return;

  const picks = [];
  for (let i = 0; i < 30; i++) {
    const p = window.sportifyGetProductById(`shop-${i}`);
  ...
  grid.innerHTML = picks
    .map((p) => {
      const url = window.sportifyProductUrl(p);
      const safeName = String(p.name || 'Product').replace(/"/g, '&quot;');
      const resolved = window.sportifyResolveProductImage ? window.sportifyResolveProductImage(p, p.image) : (p.image || '');
      ...
      return `<article class="product" data-product-id="${p.id}">
        <div class="product__media">
          <a class="product__media-link" href="${url}">
            <img src="${safeImg}" alt="${safeName}" onerror="this.onerror=null;this.src='https://placehold.co/700x860/111111/FF4D1C?font=montserrat&text=Sportify';">
          </a>
...
function hydrateFlashPrices() {
  document.querySelectorAll('#flash-sale .product[data-product-id]').forEach((article) => {
    ...
    if (!p || !wrap) return;
    const opts = window.sportifyGetSizeOptions?.(p) || [];
    const base = opts.find((x) => x.size === p.baseSize) || opts[0];
    if (!base) return;
    ...
    const now = window.sportifyFormatMoney?.(nowValue, p.currency) || '';
    const was = window.sportifyFormatMoney?.(wasValue, p.currency) || '';
...
function hydrateListingPrices() {
  document.querySelectorAll('#best-sellers .product[data-product-id]').forEach((article) => {
    ...
    const opts = window.sportifyGetSizeOptions?.(p) || [];
    const base = opts.find((x) => x.size === p.baseSize) || opts[0];
    if (base && window.sportifyFormatMoney) wrap.innerHTML = `<span class="price price--now">${window.sportifyFormatMoney(base.price, p.currency)}</span>`;
  });
}
