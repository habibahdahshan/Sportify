(function () {
  var PWD_KEY = 'sportify_admin_pwd';
  var SESS_KEY = 'sportify_admin_session';
  var ORDERS_KEY = 'sportify_orders';
  var USERS_KEY = 'sportify_users_registry';
  ...
  function getCatalogState() {
    if (window.sportifyLoadAdminCatalogState) return window.sportifyLoadAdminCatalogState();
    try {
      var raw = localStorage.getItem('sportify_admin_catalog');
  ...
  function saveCatalogState(state) {
    if (window.sportifySaveAdminCatalogState) window.sportifySaveAdminCatalogState(state);
    else localStorage.setItem('sportify_admin_catalog', JSON.stringify(state));
  }
  ...
  function renderUsers() {
    ...
    tbody.querySelectorAll('[data-remove-user]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        if (email) {
          try {
            var legacy = localStorage.getItem('sportify_user_v2');
            if (legacy) {
              var u = JSON.parse(legacy);
              if (u && String(u.email || '').toLowerCase().trim() === email) {
                localStorage.removeItem('sportify_user_v2');
              }
            }
          } catch (e1) {}
          try {
            sessionStorage.removeItem('sportify_session');
          } catch (e2) {}
        }
        renderUsers();
      });
  ...
  function renderOrderItemsReadonly(items) {
    ...
    var rows = arr
      .map(function (it) {
        var sz = it.size != null && it.size !== '—' ? 'Size ' + String(it.size) + ' · ' : '';
  ...
  function renderProducts() {
    ...
    if (!tbody || !window.sportifyGetProductById) return;
    tbody.innerHTML = '';
    var st = getCatalogState();
    var ids = window.sportifyCatalogIds ? window.sportifyCatalogIds.slice() : [];
  ...
  function clearProductForm() {
    ...
    document.getElementById('pfBaseSize').value = 'M';
  ...
  function addSizeRow(body, size, price, inStock) {
    var tbody = body || document.getElementById('pfSizesBody');
    if (!tbody) return;
    var tr = document.createElement('tr');
    tr.innerHTML =
      '<td><input type="text" class="pf-size" value="' +
      size +
      '" maxlength="4"></td>' +
  ...
  function openProductForm(productId) {
    ...
    var p = window.sportifyGetProductById ? window.sportifyGetProductById(productId) : null;
    var st = getCatalogState();
    ...
    if (catVal === 'shop' && window.sportifyInferGenderFromProduct) {
      catVal = window.sportifyInferGenderFromProduct(p);
    } else if (catVal === 'shop') {
      catVal = 'unisex';
    }
    ...
    document.getElementById('pfBaseSize').value = p.baseSize || 'M';
    document.getElementById('pfCompare').value = p.compareAt != null ? p.compareAt : '';
    var body = document.getElementById('pfSizesBody');
    if (body) {
      body.innerHTML = '';
      (p.sizes || []).forEach(function (s) {
        addSizeRow(body, s.size, s.price, s.inStock !== false);
      });
      if (!p.sizes || p.sizes.length === 0) {
        addSizeRow(body, 'M', 1000, true);
      }
    }
  }
  ...
  function buildProductPayloadFromForm(isNew) {
    ...
    var baseSize = (document.getElementById('pfBaseSize').value || 'M').trim();
    var cat = document.getElementById('pfCategory').value;
    var line =
      document.getElementById('pfLine').value.trim() ||
      (cat === 'men' ? 'Men Training Tee' : cat === 'women' ? 'Women Training Tee' : 'Unisex Training Tee');
    var payload = {
    ...
    if (cmp !== '') {
      var cmpVal = parseFloat(cmp);
      if (isNaN(cmpVal) || cmpVal < 0) {
        alert('Compare-at price cannot be negative.');
        return null;
      }
      payload.compareAt = cmpVal;
    }
    if (isNew) {
